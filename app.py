...
import torch
from stream_generator import yield_tweet
import time
...
def main():
    st.title("Privacy-Preserving Sentiment Analysis")
    
    # Sidebar for Navigation
    st.sidebar.header("Navigation")
    mode = st.sidebar.radio("Select Mode", ["Batch Analysis (File Upload)", "Real-Time Stream"])
    ...
    if mode == "Batch Analysis (File Upload)":
        st.header("Batch Analysis Simulation")
        st.markdown("""
        Upload social media data (CSV/Excel) to simulate the privacy pipeline.
        """)
        ...
        st.markdown("""
        Simulating a live social media feed. Differential Privacy is applied to each tweet's embedding in real-time.
        """)
        
        if st.button("Start Stream"):
            st.write("Streaming started... (Click 'Stop' in sidebar to halt - actually just reload page to stop for now)")
            ...
            for tweet in tweet_gen:
                ...
                # Extract Embedding
                emb = analyzer.get_embeddings(text) # Shape (1, 768)
                
                # Apply DP (Event-Level)
                # Note: clip_and_noise_embeddings expects (N, D)
                priv_emb = clip_and_noise_embeddings(emb, epsilon, clip_norm)
                
                # Classify
                true_sent = analyzer.classify_embeddings(emb)
                priv_sent = analyzer.classify_embeddings(priv_emb)
                ...
                result = {
                    ...
                    "User": tweet['username'],
                    "Comment": text,
                    "True Sentiment": true_sent,
                    "Private Sentiment": priv_sent
                }
                stream_data.append(result)
                
                # Keep last 20 tweets for feed
                recent_df = pd.DataFrame(stream_data[-20:])
                feed_placeholder.dataframe(recent_df[['Time', 'User', 'Comment', 'True Sentiment', 'Private Sentiment']], hide_index=True)
                
                # Update Charts
                # Calculate cumulative counts or sliding window counts
                full_df = pd.DataFrame(stream_data)
                
                # Let's show Positive Sentiment Count over time (True vs Private)
                # We can group by index (time step) or just plot cumulative sum of 'Positive'
                
                full_df['True Positive'] = (full_df['True Sentiment'] == 'Positive').astype(int)
                full_df['Private Positive'] = (full_df['Private Sentiment'] == 'Positive').astype(int)
                
                full_df['Cumulative True Positive'] = full_df['True Positive'].cumsum()
                full_df['Cumulative Private Positive'] = full_df['Private Positive'].cumsum()
                
                chart_data = full_df[['Cumulative True Positive', 'Cumulative Private Positive']].reset_index(drop=True)
